...
/*
PROC IMPORT OUT= WORK.BCA_CodeList_NonFees 
            DATAFILE= "C:\inetpub\wwwroot\sasweb\Data\Temp\UML\BCA_CodeList_NonFees.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;

PROC IMPORT OUT= WORK.BCA_CodeList_Fees 
            DATAFILE= "C:\inetpub\wwwroot\sasweb\Data\Temp\UML\BCA_CodeList_Fees.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
*/
    ...
    infile 'C:\inetpub\wwwroot\sasweb\Data\Temp\V2_2\UML\BCA_CodeList_NonFees.csv'
...
data WORK.BCA_CODELIST_FEES;
    %let _EFIERR_ = 0; /* set the ERROR detection macro variable */
    infile 'C:\inetpub\wwwroot\sasweb\Data\Temp\V2_2\UML\BCA_CodeList_Fees.csv' 
	delimiter = ',' MISSOVER DSD lrecl=32767 firstobs=2 TermStr = CRLF;
       informat CodelistName $50. ;
...
Data Work.BCA_CodeList_Fees1(Drop = Description);
	Set Work.BCA_CodeList_Fees;
	CodeDescription = Description;
	If Include_in_v2_0_ EQ 'N' or CodeName EQ '' then Delete;
/*	Where CodeListName = 'OB_CardType1Code';*/
Run;

Data Work.BCA_CodeList_NonFees1(Drop = Description Rename=(EndPoint_CodeName = CodeName));
	Set Work.BCA_CodeList_NonFees;
	CodeDescription = Description;
...
Data OBData.BCA_CodeList;
	Length CodelistName $ 50 CodeName CodeDescription $ 1000;
	Set Work.BCA_CodeList_Fees1(In=a Drop = Notes)
	Work.BCA_CodeList_NonFees1(In=b Drop = Notes);
	Length Infile $ 8;
...
Proc Sort Data = OBData.BCA_CodeList(Keep = CodeName CodeListName CodeDescription
...
*--- Get data from API_BCA Data Dictionary data ---;
...
%Mend Import;
%Import(C:\inetpub\wwwroot\sasweb\Data\Temp\V2_2\UML\bcal_001_001_01DD.csv,BCA);
...
Data Work.API_BCA1(Drop = CodeListName Rename = (NewVar = CodeListName));
	Set Work.API_BCA;
...
Proc Sort Data = Work.API_BCA1
		Out = OBData.API_BCA2 NoDupKey;
	By CodeListName CodeName CodeDescription;
Run;

Data OBData.API_BCA2;
	Set OBData.API_BCA2;
...
Data OBData.BCA_Code_Compare Work.BCA_Code_Compare;
	Length Count 4 Hierarchy $ 1000 CodeListName $ 50 CodeName CodeDescription $ 1000;

	Merge OBData.API_BCA2(In=b Keep = Hierarchy CodeListName CodeName CodeDescription
	CodeListName_DD CodeName_DD CodeDescription_DD)

	OBData.BCA_CodeList(In=a Keep = CodeListName CodeName CodeDescription
...
Proc Report Data = OBData.BCA_Code_Compare nowd
...
Proc Export Data = OBData.BCA_Code_Compare
 	Outfile = "C:\inetpub\wwwroot\sasweb\Data\Results\BCA_CodeList_DD_Comparison_Final.csv"
	DBMS = CSV REPLACE;
	PUTNAMES=YES;
Run;
...
Proc Report Data = OBData.BCA_Code_Compare(Where=(Infile='Both')) nowd
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black];
...
Proc Report Data = OBData.BCA_Code_Compare(Where=(Infile='CodeList')) nowd
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black];
...
Proc Export Data = OBData.BCA_Code_Compare(Where=(Infile='CodeList'))
 	Outfile = "C:\inetpub\wwwroot\sasweb\Data\Results\BCA_CodeList_DD_Comparison.csv"
	DBMS = CSV REPLACE;
	PUTNAMES=YES;
Run;
...
Proc Report Data = OBData.BCA_Code_Compare(Where=(Infile='DD')) nowd
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black];
