...
Data WORK.SME_CODELIST_FEES;
    ...
    infile 'C:\inetpub\wwwroot\sasweb\Data\Temp\od\ob\V2_2\SME_CodeList_Fees.csv' 
...
Data Work.SME_CodeList_Fees1(Drop = Description);
	Set Work.SME_CodeList_Fees;
...
Data OBData.SME_CodeList;
	Length CodelistName $ 50 CodeName CodeDescription $ 1000;
	Set Work.SME_CodeList_Fees1(In=a Drop = Notes);
	Length Infile $ 8;
...
Proc Sort Data = OBData.SME_CodeList(Keep = CodeName CodeListName CodeDescription
...
*--- Get data from API_SME Data Dictionary data ---;
...
%Mend Import;
%Import(C:\inetpub\wwwroot\sasweb\Data\Temp\od\ob\V2_2\SMEl_001_001_01DD.csv,SME);
...
Data Work.API_SME1(Drop = CodeListName Rename = (NewVar = CodeListName));
	Set Work.API_SME;
...
Proc Sort Data = Work.API_SME1
		Out = OBData.API_SME2 NoDupKey;
	By CodeListName CodeName CodeDescription;
Run;

Data OBData.API_SME2;
	Set OBData.API_SME2;
...
Data OBData.SME_Code_Compare Work.SME_Code_Compare;
	Length Count 4 Hierarchy $ 1000 CodeListName $ 50 CodeName CodeDescription $ 1000;

	Merge OBData.API_SME2(In=b Keep = Hierarchy CodeListName CodeName CodeDescription
	CodeListName_DD CodeName_DD CodeDescription_DD)

	OBData.SME_CodeList(In=a Keep = CodeListName CodeName CodeDescription
...
Proc Report Data = OBData.SME_Code_Compare nowd/*
...
Proc Export Data = OBData.SME_Code_Compare
 	Outfile = "C:\inetpub\wwwroot\sasweb\Data\Results\SME_CodeList_DD_Comparison_Final_%Sysfunc(UPCASE(&Fdate)).csv"
	DBMS = CSV REPLACE;
	PUTNAMES=YES;
Run;
...
Proc Report Data = OBData.SME_Code_Compare(Where=(Infile='Both')) nowd/*
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black]*/;
...
Proc Report Data = OBData.SME_Code_Compare(Where=(Infile='CodeList')) nowd/*
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black]*/;
...
Run; 
/*
Proc Export Data = OBData.SME_Code_Compare(Where=(Infile='CodeList'))
 	Outfile = "C:\inetpub\wwwroot\sasweb\Data\Results\SME_CodeList_DD_Comparison.csv"
	DBMS = CSV REPLACE;
	PUTNAMES=YES;
Run;
*/
...
Proc Report Data = OBData.SME_Code_Compare(Where=(Infile='DD')) nowd/*
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black]*/;
