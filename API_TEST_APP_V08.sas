...
*--- Close Output Delivery Parameters  ---;
	...
	%No_Errors(&Bank, &API);
	    ...
	    Put '<INPUT TYPE=hidden NAME=WebUser VALUE="vamola@mac.com">';
	    Put '<INPUT TYPE=hidden NAME=WebPass VALUE="Test">';
		Put '</Form>';
		Put '</td>';
		Put '</tr>';
		Put '</Table>';
...
%Mend Schema;
%Schema(&GitHub_Path/%Sysfunc(Tranwrd(&Github_API..json,'-','_')),&Bank_Name);
