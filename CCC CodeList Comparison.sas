...
Data Work.CCC_CodeList_Fees1(Drop = Description);
	Set Work.CCC_CodeList_Fees;
...
Data OBData.CCC_CodeList;
	Length CodelistName $ 50 CodeName CodeDescription $ 1000;
	Set Work.CCC_CodeList_Fees1(In=a Drop = Notes);
	Length Infile $ 8;
...
Proc Sort Data = OBData.CCC_CodeList(Keep = CodeName CodeListName CodeDescription
...
*--- Get data from API_CCC Data Dictionary data ---;
...
Data Work.API_CCC1(Drop = CodeListName Rename = (NewVar = CodeListName));
	Set Work.API_CCC;
...
Proc Sort Data = Work.API_CCC1
		Out = OBData.API_CCC2 NoDupKey;
	By CodeListName CodeName CodeDescription;
Run;

Data OBData.API_CCC2;
	Set OBData.API_CCC2;
...
Data OBData.CCC_Code_Compare Work.CCC_Code_Compare;
	...
	Merge OBData.API_CCC2(In=b Keep = Hierarchy CodeListName CodeName CodeDescription
	CodeListName_DD CodeName_DD CodeDescription_DD)

	OBData.CCC_CodeList(In=a Keep = CodeListName CodeName CodeDescription
...
Proc Report Data = OBData.CCC_Code_Compare(Where=(Infile='CodeList')) nowd/*
...
Proc Report Data = OBData.CCC_Code_Compare(Where=(Infile='DD')) nowd/*
	style(report)=[width=100%]
	style(report)=[rules=all cellspacing=0 bordercolor=gray] 
	style(header)=[background=lightskyblue foreground=black] 
	style(column)=[background=lightcyan foreground=black]*/;
